    ...
    <div className="w-full h-screen bg-gray-800 flex flex-col items-center justify-start p-1 sm:px-5 pt-24 gap-5 text-gray-100">
      ...
      <div className="relative w-full h-full border rounded-sm">
        <div onClick={previousSlide}>
          <BsChevronLeft onClick={() => setShowAnswer(false)} className="absolute top-[50%] left-0 sm:left-6 z-10 -translate-y-1/2 w-10 h-10 cursor-pointer hover:scale-110 active:scale-90 transition-all" />
        </div>
        <div onClick={nextSlide}>
          <BsChevronRight onClick={() => setShowAnswer(false)} className="absolute top-[50%] right-0 sm:right-6 z-10 -translate-y-1/2 w-10 h-10 cursor-pointer hover:scale-110 active:scale-90 transition-all" />
        </div>
